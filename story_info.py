...
class EmptyBot:
    def __init__(self, player):
        ...
        self.language = None
        self.story_lines = None
        ###########################################  'end:' 라는 패턴으로 엔딩을 저장해두었을 때 아래처럼 엔딩의 개수를 구할 수 있다.
        self.ending_numbers = None
        ###########################################
...
class PrisonStoryBot(EmptyBot):
    ...
    def update_story(self):  # 이게 조건 확인하는 복잡한 함수
        ...
        if self.next_story_is_ready():
            if self.progress_check(0) and self.have_revisited_StartingRoom:  # [시작의 방에 처음으로 돌아왔을 때]
            ...
            elif self.progress_check(2) and self.scan_item_of_type_in_inv(
                    items.Magical):  # [Magical weapon을 얻거나 발견한다 - 직후]
                # print('success 3')
                self.process_story_txt_and_push(self.story_lines[2])
                self.progress_to_next_story()  # 3으로 넘어감

            elif self.progress_check(
                    3) and self.find_charming_item():  # [charming type Magical weapon를 얻거나 발견한 적이 있을 때]
                # print('success 4')
                ...
                self.progress_to_next_story()  # 5로 넘어감
                    ...
                    5) and self.player_MagicAffinity_level == statuses.Status.max_level and self.is_player_on_memory_tile():  # [Magic affinity가 max level인 상태로 메모리 타일에 도달]
                # print('success 6')
                self.process_story_txt_and_push(self.story_lines[5])
                self.progress_to_next_story()  # 6로 넘어감

            elif self.progress_check(
                    6) and self.have_been_to_LeaveCaveRoom and self.is_player_on_memory_tile():  # [동굴 출구에 도달한 적이 있으나, 나가지 않고 메모리 타일로 왔을 때]
                # print('success 7')
...
Creatures I killed... they weren't just imaginary thiefs, magicians, guards...
