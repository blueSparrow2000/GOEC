...
class MapTile:  # abstract class
    def __init__(self, x, y, name):
        ...
        self.peaceful_state_actions = [actions.ViewInventory(), actions.ViewStatus(), actions.ViewMobpedia(),
                                       actions.ViewMinimap(), actions.Eat()]
    ...
    def adjacent_moves(
            self):  # 주의. 이 함수를 init에 넣지 않는 이유가 있음. tile은 world를 load할때 '생성'되는데, 그 동안에는 옆에 타일이 있어도, 현재 타일이 더 먼저 생성되면 옆에 타일은 아직 없는 것 처럼 파악됨. 그래서 생성자에서 바로 self.moves를 하면 안됨!
...
class EnemyRoom(MapTile):  # abstract class
    def __init__(self, x, y, name, enemy):
        ...
        self.engage_actions = [actions.Eat(), actions.Flee(tile=self), actions.Attack(enemy=self.enemy),
                               actions.AttackPreviousOption(enemy=self.enemy)]
    ...
    def engage(self, the_player):
        ...
        if self.enemy.is_alive():
            ...
            if self.enemy.skills == 0 or skill_no == 0:
                if the_player.take_enemy_damage(self.enemy.damage):
                    print("Enemy does {} damage.\n{} HP: \033[91m{}\033[0m".format(round(self.enemy.damage, 1),
                                                                                   the_player.name,
                                                                                   round(the_player.hp, 1)))
...
class NPCRoom(EnemyRoom):  # abstract class
    def __init__(self, x, y, name, npc):
        ...
        self.npc_actions = [actions.Attack(enemy=self.enemy), actions.AttackPreviousOption(enemy=self.enemy),
                            actions.Talk(npc=self.enemy), actions.Sell(npc=self.enemy)]
...
class FindKeyRoom(LootRoom):
    def __init__(self, x, y, name, key_address_code):
        self.item = items.Key(key_address_code)
        super().__init__(x, y, name, self.item)
...
class GandalphRoom(EnemyRoom):
    def __init__(self, x, y, name):
        ...
        self.engage_actions = [actions.Eat(), actions.Attack(enemy=self.enemy),
                               actions.AttackPreviousOption(enemy=self.enemy)]  # cannot flee in front of Gandalph!
